...
function timeview(timenum){
	...
	if(_thisTime.getTime()-_time.getTime()<60){
		...
		if((_time.getMonth()==_thisTime.getMonth())&&(_time.getDate()==_thisTime.getDate())){
			...
			return _time.getMonth()+'月'+_time.getDate()+'日'+_time.getHours()+':'+(nu=function(num){if(num<10)return '0'+num;else return ''+num;})(_time.getMinutes())+':'+nu(_time.getSeconds());
		}
        ...
        dataBase.transaction(function(ts){
            ts.executeSql(sql, [], function(ts, result){
				/*******************************************/
					...
					res=window.res;
					if(res.length){
							$thissqltable=$(document.createElement('div')).append('<h3 class="with-padding bg-success" onclick="slide(this)"  style="cursor:pointer">运行成功<code>'+$("#sqlbox").val()+'</code></h3>');
							...
							$thissqltable=$(document.createElement('div')).prepend('<div class="alert alert-info"><strong>通知！</strong> 运行SQL：<code>'+$("#sqlbox").val()+'</code>完成,没有返回数据</div>');
                ...
                function(ts, error){
						...
						$thissqltable='<div class="alert alert-danger"><strong>警告！</strong> 运行SQL：“'+$("#sqlbox").val()+'”出现了问题，如果你的SQL没有错误，请尝试重新启动浏览器！<p>'+error.message+'</p></div>'
